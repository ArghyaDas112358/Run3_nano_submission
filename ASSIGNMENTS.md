...
| `WJetsLO` | 9 | large (1.83 B evts) | Arghya (arghyara) | **submitted** | **the analysis background** (W→lnu 4 + Wto2Q HT 5). Full round 2026-08-05, tasks `260805_2046`–`260805_2049`; **9/9 SUBMITTED, 8 RUNNING** read back from the server. `WtoLNu-4Jets_Bin-4J` had been SUBMITREFUSED on the self-test (>100k lumis in one block) and now carries the `FileBased` override from `cards/chs_mc.yml` |
